...
class CFrontendSetup {
	...
	public function checkPhpMaxExecutionTime() {
		...
		return [
			...
			'error' => _s('Minimum required limit on execution time of PHP scripts is %1$s (configuration option "max_execution_time").', self::MIN_PHP_MAX_EXECUTION_TIME)
		];
	}
	...
	public function checkPhpMaxInputTime() {
		...
		return [
			...
			'result' => $currentIsValid ? self::CHECK_OK : self::CHECK_FATAL,
			'error' => _s('Minimum required limit on input parse time for PHP scripts is %1$s (configuration option "max_input_time").', self::MIN_PHP_MAX_INPUT_TIME)
		];
	}
